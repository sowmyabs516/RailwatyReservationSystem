...
if(user==null){
	resp.getWriter().print("<h1>Session Already Expired</h1>");
	...
	Train train = trainDao.fetch(tnumber);
	...
	if(seats<1){
		resp.getWriter().print("<h1>Seats cannot be less than one</h1>");
		req.getRequestDispatcher("UserHome.html").include(req, resp);
	}
	else{
		if(from.equals(to)){
			resp.getWriter().print("<h1>Source and Destination Cannot Be Same</h1>");
			req.getRequestDispatcher("UserHome.html").include(req, resp);
		}
		else {
			int fromPos = 0;
				...
				if (Period.between(dob.toLocalDate(), doj.toLocalDate()).getDays() < 0 || flag) {
					...
					if (user.getWallet() < amount) {
						...
						if (train.getSeat() < seats) {
							...
							ticket.setTrainNumber(tnumber);
							ticket.setStatus("Booked");

							trainDao.save(ticket);
